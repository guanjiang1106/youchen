...
from module_business.service.process_service import ProcessService
from module_business.entity.vo.process_vo import DeleteProcessModel, ProcessModel, ProcessPageQueryModel
...
process_controller = APIRouterPro(
    prefix='/module_business/process', order_num=50, tags=['工艺'], dependencies=[PreAuthDependency()]
)
...
@process_controller.get(
    ...
    dependencies=[UserInterfaceAuthDependency('module_business:process:list')],
)
async def get_module_business_process_list(
    request: Request,
...
@process_controller.post(
    ...
    dependencies=[UserInterfaceAuthDependency('module_business:process:add')],
)
@ValidateFields(validate_model='add_process')
@Log(title='工艺', business_type=BusinessType.INSERT)
async def add_module_business_process(
    request: Request,
...
@process_controller.put(
    ...
    description='用于编辑工艺',
    response_model=ResponseBaseModel,
    dependencies=[UserInterfaceAuthDependency('module_business:process:edit')],
)
@ValidateFields(validate_model='edit_process')
@Log(title='工艺', business_type=BusinessType.UPDATE)
async def edit_module_business_process(
    request: Request,
...
@process_controller.delete(
    ...
    description='用于删除工艺',
    response_model=ResponseBaseModel,
    dependencies=[UserInterfaceAuthDependency('module_business:process:remove')],
)
@Log(title='工艺', business_type=BusinessType.DELETE)
async def delete_module_business_process(
    request: Request,
...
@process_controller.get(
    ...
    dependencies=[UserInterfaceAuthDependency('module_business:process:query')]
)
async def query_detail_module_business_process(
    request: Request,
...
@process_controller.post(
    ...
    dependencies=[UserInterfaceAuthDependency('module_business:process:export')],
)
@Log(title='工艺', business_type=BusinessType.EXPORT)
async def export_module_business_process_list(
    request: Request,
