...
@ai_model_controller.post(
    '/test',
    summary='测试AI模型连接接口',
    description='用于测试AI模型配置是否正确',
    response_model=ResponseBaseModel,
    dependencies=[UserInterfaceAuthDependency('ai:model:query')],
)
async def test_ai_model_connection(
    request: Request,
    test_model: AiModelModel,
    query_db: Annotated[AsyncSession, DBSessionDependency()],
) -> Response:
    test_result = await AiModelService.test_ai_model_connection_services(query_db, test_model)
    logger.info(test_result.message)

    return ResponseUtil.success(msg=test_result.message)
