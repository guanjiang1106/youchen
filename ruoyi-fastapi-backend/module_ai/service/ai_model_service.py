...
class AiModelService:
    ...
    @classmethod
    async def test_ai_model_connection_services(
        cls, query_db: AsyncSession, test_model: AiModelModel
    ) -> CrudResponseModel:
        """
        测试AI模型连接service

        :param query_db: orm对象
        :param test_model: 测试模型对象
        :return: 测试结果
        """
        try:
            # 这里可以根据不同的提供商进行实际的API调用测试
            # 目前先返回成功，后续可以集成实际的AI SDK进行测试
            if not test_model.api_key:
                raise ServiceException(message='API Key不能为空')
            if not test_model.model_code:
                raise ServiceException(message='模型编码不能为空')
            if not test_model.provider:
                raise ServiceException(message='提供商不能为空')

            # TODO: 根据不同提供商调用对应的API进行测试
            # 例如：
            # if test_model.provider == 'OpenAI':
            #     await test_openai_connection(test_model)
            # elif test_model.provider == 'DeepSeek':
            #     await test_deepseek_connection(test_model)

            return CrudResponseModel(is_success=True, message='连接测试成功')
        except ServiceException as e:
            raise e
        except Exception as e:
            raise ServiceException(message=f'连接测试失败: {str(e)}')
