...
class GenTable(Base):
    ...
    form_layout = Column(String(5000), nullable=True, comment='表单布局配置（JSON格式）')
...
class GenTableColumn(Base):
    ...
    link_table = Column(String(200), nullable=True, comment='关联表名称')
    link_label_field = Column(String(200), nullable=True, comment='关联表标签字段')
    link_value_field = Column(String(200), nullable=True, comment='关联表值字段')
    tab_page = Column(String(50), nullable=True, server_default='basic', comment='显示页签（basic基本信息/detail详细信息）')
    default_value = Column(String(500), nullable=True, comment='默认值')
    example_value = Column(String(500), nullable=True, comment='示例值')
    list_width = Column(Integer, nullable=True, comment='列表显示宽度（像素）')
