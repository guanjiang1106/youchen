...
@gen_controller.post(
    '/generateTableSQL',
    summary='AI生成建表SQL接口',
    description='用于通过AI模型生成建表SQL语句',
    response_model=DataResponseModel[str],
    dependencies=[RoleInterfaceAuthDependency('admin')],
)
@Log(title='AI生成SQL', business_type=BusinessType.OTHER)
async def generate_table_sql(
    request: Request,
    model_id: Annotated[int, Query(description='模型ID', alias='modelId')],
    requirement: Annotated[str, Query(description='表需求描述')],
    query_db: Annotated[AsyncSession, DBSessionDependency()],
) -> Response:
    generate_sql_result = await GenTableService.generate_table_sql_services(query_db, model_id, requirement)
    logger.info('AI生成SQL成功')

    return ResponseUtil.success(data=generate_sql_result)
