    ...
    async def generate_random_teacher_services(cls, query_db: AsyncSession, count: int) -> CrudResponseModel:
        ...
        try:
            # 定义随机数据源
            departments = ['计算机学院', '数学学院', '物理学院', '化学学院', '生物学院', '文学院', '历史学院', '哲学学院']
            titles = ['讲师', '副教授', '教授', '助教', '研究员']
            gender_values = [0, 1]
            status_values = [0, 1]
            
            teachers = []
            base_date = datetime.now() - timedelta(days=365*20)
            
            for i in range(count):
                # 生成随机数据
                first_name = random.choice(['张', '王', '李', '赵', '刘', '陈', '杨', '黄', '周', '吴'])
                last_name = random.choice(['伟', '芳', '娜', '敏', '静', '丽', '强', '磊', '军', '洋'])
                name = f"{first_name}{last_name}"
                teacher_no = f"T{datetime.now().strftime('%Y%m%d%H%M%S')}{str(i).zfill(3)}"
